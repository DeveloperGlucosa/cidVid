$(document).ready( function(){
    let menuButton = $('.menu-button'), navMenu = $('.menu');
    ...
    $('.timelime-item').each(function(){
        $(this).on("click", function(){
            $("p.timeline-text").css({"display": "none"});
...
var nosotrosTween =  gsap.timeline()
    .from("#nosotros .imagen", .5, {x: -180, scale: 0.7, opacity: 0})
    .from("#nosotros .texto", .5, {x: 180, scale: 0.7, opacity: 0})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#nosotros"
})
.setTween(nosotrosTween)
.addTo(controller);

var historiaTween =  gsap.timeline()
    .from("#historia .historia-title", .5, {x: -180, scale: 0.7, opacity: 0})
    .from("#historia .description", .5, {x: 180, scale: 0.7, opacity: 0})
    .from(".timelime-item", .80, {y:-20,scale: 0.3, opacity:0, stagger: 0.25})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#historia"
})
.setTween(historiaTween)
.addTo(controller);

var divisionTween =  gsap.timeline()
    .from("#divisiones h2", .5, {x: -180, scale: 0.7, opacity: 0})
    .from("#divisiones p", .5, {x: 180, scale: 0.7, opacity: 0})
    .from(".landing-button", .80, {y:10, scale: 0.3, opacity:0, stagger: 0.25})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#divisiones"
})
.setTween(divisionTween)
.addTo(controller);

var enfocamosTween =  gsap.timeline()
    .from("#nos-enfocamos p", .5, {y: 180, scale: 0.5, opacity: 0})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#nos-enfocamos"
})
.setTween(enfocamosTween)
.addTo(controller);

var mapaTween =  gsap.timeline()
    .from("#instalaciones .texto", .5, {x: -180, opacity: 0})
    .from("#instalaciones .mapa", .5, {x: 180, opacity: 0})
new ScrollMagic.Scene({
    offset: 50,
...
var clientesTween =  gsap.timeline()
    .from("#clientes h2", .5, {y: -180, opacity: 0})
    .from("#clientes p", .5, {y: 180, opacity: 0})
    .from("#clientes .clientes-img", .8, {y: -180, scale:.5, opacity: 0})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#clientes"
})
.setTween(clientesTween)
.addTo(controller);


var inspiramosTween =  gsap.timeline()
    .from("#inspiramos p", .5, {y: -180, scale: 0.5, opacity: 0})
new ScrollMagic.Scene({
    offset: 50,
    triggerElement: "#inspiramos"
})
.setTween(inspiramosTween)
.addTo(controller);

var contactoTween =  gsap.timeline()
    .from("#contacto h2.title", .5, {y: -180, opacity: 0})
    .from("#contacto p.text", .5, {y: 180, opacity: 0})
    .from("#contacto .form-control", 1, {y: 180, opacity: 0, stagger: .25})
new ScrollMagic.Scene({
    offset: 50,
